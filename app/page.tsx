...
import {
  Activity, ArrowDown, ArrowUp, ArrowUpDown, BarChart3, Bitcoin, Building2, CandlestickChart, ChevronRight, Clock3, Coins,
  ExternalLink, Gauge, Globe2, Layers3, LayoutDashboard, ListFilter, Radar, RefreshCw, Search, Settings,
  Square, Star, Target, TrendingDown, TrendingUp, Zap,
} from "lucide-react";
...
type Stock = {
  ...
  annualDividend?: number; dividendYield?: number;
...
const nyseSymbols = new Set(["BABA", "BRK.B", "CAT", "CVX", "HD", "IBM", "JNJ", "JPM", "KO", "LLY", "LOW", "MCD", "NVO", "PG", "TSM", "V", "WMT", "XOM"]);
function tradingViewUrl(stock: Stock, fallbackMarket: MarketKey) {
  const market = stock.marketKey ?? fallbackMarket;
  const symbol = market === "thai" ? `SET:${stock.symbol}`
    : market === "crypto" ? `BINANCE:${stock.symbol}USDT`
      : market === "etf" ? `AMEX:${stock.symbol}`
        : `${nyseSymbols.has(stock.symbol) ? "NYSE" : "NASDAQ"}:${stock.symbol}`;
  return `https://www.tradingview.com/chart/?symbol=${encodeURIComponent(symbol)}`;
}

const brandLogoSlugs: Record<string, string> = {
  BTC: "bitcoin", ETH: "ethereum", SOL: "solana", BNB: "binance", XRP: "xrp", ADA: "cardano",
  DOGE: "dogecoin", AVAX: "avalanche", LINK: "chainlink", DOT: "polkadot", LTC: "litecoin", BCH: "bitcoincash",
...
function AssetIcon({ stock, fallbackMarket, thaiLogoUrl, large = false }: { stock: Stock; fallbackMarket: MarketKey; thaiLogoUrl?: string; large?: boolean }) {
  const market = stock.marketKey ?? fallbackMarket;
  ...
  const imageUrl = thaiLogoUrl ?? (slug
  ...
  return <div className={`ticker-logo asset-icon ${market} ${thaiDomain || thaiLogoUrl ? "thai-brand" : ""} ${large ? "large" : ""}`} title={`${stock.symbol} · ${stock.name}`}>
    {market === "thai" ? <span className="asset-symbol-fallback">{stock.symbol.replace(/[^A-Z0-9]/g, "").slice(0, 2)}</span> : <FallbackIcon className="asset-category-icon"/>}
    {imageUrl && (
      <img src={imageUrl} alt={`${stock.symbol} logo`} loading="lazy" decoding="async" onError={(event) => { event.currentTarget.style.display = "none"; }}/>
    )}
  </div>;
}
...
export default function Home() {
  ...
  const [thaiLogos, setThaiLogos] = useState<Record<string, string>>({});
  ...
  const displayStocks = stocks.length ? stocks : marketKey === "thai" && theme === "core" && !scanning ? demoStocks : [];
  ...
  async function scan(nextBatch = 0, append = false, search = query, runToken?: number) {
    const token = runToken ?? ++scanRunRef.current;
    ...
    try {
      ...
      if (token !== scanRunRef.current) return;
      ...
      const hasNextBatch = allThai && !search && data.universe && data.universe.batch + 1 < data.universe.totalBatches;
      if (hasNextBatch) {
        await new Promise((resolve) => setTimeout(resolve, 550));
        if (token === scanRunRef.current) await scan(data.universe!.batch + 1, true, "", token);
      }
    } catch (scanError) {
      setMarket(null); setSource("แหล่งข้อมูลฟรีขัดข้อง");
      setError(scanError instanceof Error ? scanError.message : "โหลดข้อมูลไม่สำเร็จ");
    } finally {
      if (token === scanRunRef.current) setScanning(false);
    }
  }
  ...
  function stopScan() {
    ...
    setScanning(false);
    setSource("หยุดการสแกนชั่วคราว · กดสแกนเพื่อเริ่มใหม่");
  }

  useEffect(() => { void loadPulse(); }, []);
  useEffect(() => { void scan(0, false, ""); }, [timeframe, marketKey, theme, thaiUniverse]);
  useEffect(() => {
    if (marketKey !== "thai" || Object.keys(thaiLogos).length) return;
    let active = true;
    fetch("/api/thai-logos").then((response) => response.ok ? response.json() : null)
      .then((data: { logos?: Record<string, string> } | null) => { if (active && data?.logos) setThaiLogos(data.logos); })
      .catch(() => undefined);
    return () => { active = false; };
  }, [marketKey, thaiLogos]);
  useEffect(() => {
          ...
          <div className="content">
            {viewMode === "backtest" ? <BacktestLab/> : <>
              ...
              <section className="scanner-panel">
                ...
                {marketKey === "thai" && theme === "core" && thaiUniverse === "all" && <div className="batch-toolbar"><div><b>จักรวาลหุ้นไทย {universeMeta.total.toLocaleString("th-TH")} ตัว</b><span>{scanning ? "กำลังโหลดอัตโนมัติ" : batch + 1 >= universeMeta.totalBatches ? "สแกนครบแล้ว" : "หยุดไว้ชั่วคราว"} · สำเร็จ {scanned.toLocaleString("th-TH")} ตัว · ชุด {batch + 1}/{universeMeta.totalBatches}</span></div><div className="batch-progress"><i style={{ width: `${Math.min(100, (batch + 1) / universeMeta.totalBatches * 100)}%` }}/></div><strong>{Math.round(Math.min(100, (batch + 1) / universeMeta.totalBatches * 100))}%</strong></div>}
              ...
              {selected && <section className="detail-panel">
                <div className="detail-head"><div className="stock-title"><AssetIcon stock={selected} fallbackMarket={marketKey} thaiLogoUrl={marketKey === "thai" ? thaiLogos[selected.symbol] : undefined} large/><div><h2>{selected.symbol} <span>{selected.name}</span></h2><p>{selected.sector} · {marketMeta.label} · {selected.currency ?? marketMeta.currency}</p></div></div><div className="detail-quote-actions"><a className="tv-detail-link" href={tradingViewUrl(selected, marketKey)} target="_blank" rel="noreferrer"><ExternalLink/>TradingView</a><Button variant="outline" size="icon-sm" aria-label="เพิ่มรายการติดตาม" onClick={() => toggleWatch(selected)}><Star className={isWatched(selected) ? "starred" : ""}/></Button><div className="quote"><strong>{fmtPrice(selected.price)}</strong><span className={selected.change >= 0 ? "positive" : "negative"}>{selected.change >= 0 ? <TrendingUp/> : <TrendingDown/>}{selected.change >= 0 ? "+" : ""}{selected.change.toFixed(2)}%</span></div></div></div>
                <div className="detail-grid"><div className="chart-card"><div className="chart-toolbar"><div><b>INTERACTIVE CHART</b><span>TradingView-style · {marketMeta.currency}</span></div><div className="ranges">{["1D", "1W", "1M", "3M"].map((item) => <button className={range === item ? "active" : ""} onClick={() => setRange(item)} key={item}>{item}</button>)}</div></div><TradingChart stock={selected} range={range} timeframe={timeframe}/></div><aside className="analysis-card">
                  ...
                  {theme === "dividend" && <div className="dividend-card"><Coins/><div><small>TRAILING 12-MONTH DIVIDEND</small><strong>{(selected.dividendYield ?? 0).toFixed(2)}%</strong><span>จ่ายย้อนหลังรวม {fmtPrice(selected.annualDividend ?? 0)} {selected.currency ?? marketMeta.currency} ต่อหน่วย</span></div></div>}
                  ...
                  <p className="disclaimer">ข้อมูลจากแหล่งฟรีอาจหน่วงและต่างกันตามตลาด · Yield TTM เป็นข้อมูลย้อนหลัง ไม่รับประกันการจ่ายครั้งถัดไป · เพื่อการศึกษา ไม่ใช่คำแนะนำซื้อขาย</p>
                </aside></div>
              </section>}
            </>}
          </div>
