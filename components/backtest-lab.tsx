...
export function BacktestLab() {
  ...
  async function runBacktest() {
    ...
    try {
      const response = await fetch(`/api/backtest?months=${months}&score=${score}`, { cache: "no-store" });
  ...
  return <section className="backtest-lab">
    ...
    {loading && !data ? <div className="backtest-loading">{Array.from({length:6}).map((_,index)=><Skeleton key={index} className="h-28 rounded-xl bg-[#172129]"/>)}</div> : data && <>
      <div className="method-strip"><ShieldCheck/><span><b>MTF Conservative:</b> ใช้เฉพาะแท่งที่ปิดแล้ว · เข้า Open ของ H1 ถัดไป · SL 1.2 ATR · TP 2R · ต้นทุน 0.15% · ชนพร้อมกันให้นับ SL</span><em>ข้อมูลครบ {data.coverage.succeeded}/{data.coverage.requested} หุ้น</em></div>
