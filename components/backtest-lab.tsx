...
type BacktestData = {
  assumptions: { years: number; timeframe: string; minScore: number; signal: string; rr: number; atrStop: number; maxTradesPerDay: number; holdingBars: number; feeRate: number; entry: string; sameBarRule: string };
...
export function BacktestLab() {
  ...
  async function runBacktest() {
    ...
    try {
      const response = await fetch(`/api/backtest?years=${years}&score=${score}&signal=${encodeURIComponent(signal)}`, { cache: "no-store" });
      ...
      setData(result);
    } catch (backtestError) { setError(backtestError instanceof Error ? backtestError.message : "Backtest ไม่สำเร็จ"); }
  ...
  return <section className="backtest-lab">
    <div className="backtest-hero"><div><p className="section-kicker"><FlaskConical/> WALK-FORWARD SIMULATION</p><h2>Backtest Lab <span>Beta</span></h2><p>พิสูจน์กฎ Scanner ด้วยข้อมูลย้อนหลัง ก่อนนำไปใช้แจ้งเตือนหรือเข้าเทรดจริง</p></div><div className="backtest-controls"><Select value={years} onValueChange={setYears}><SelectTrigger aria-label="ระยะเวลาทดสอบ"><SelectValue/></SelectTrigger><SelectContent><SelectItem value="1">ย้อนหลัง 1 ปี</SelectItem><SelectItem value="3">ย้อนหลัง 3 ปี</SelectItem><SelectItem value="5">ย้อนหลัง 5 ปี</SelectItem></SelectContent></Select><Select value={score} onValueChange={setScore}><SelectTrigger aria-label="คะแนนขั้นต่ำ"><SelectValue/></SelectTrigger><SelectContent><SelectItem value="60">คะแนน 60+</SelectItem><SelectItem value="70">คะแนน 70+</SelectItem><SelectItem value="80">คะแนน 80+</SelectItem></SelectContent></Select><Select value={signal} onValueChange={setSignal}><SelectTrigger aria-label="ประเภทสัญญาณ"><SelectValue/></SelectTrigger><SelectContent><SelectItem value="all">ทุกสัญญาณ</SelectItem><SelectItem value="Breakout">Breakout</SelectItem><SelectItem value="Pullback">Pullback</SelectItem><SelectItem value="Momentum">Momentum</SelectItem></SelectContent></Select><Button onClick={() => void runBacktest()} disabled={loading}><RefreshCw className={loading ? "spin" : ""}/>{loading ? "กำลังคำนวณ" : "เริ่มทดสอบ"}</Button></div></div>
    ...
    {loading && !data ? <div className="backtest-loading">{Array.from({length:6}).map((_,index)=><Skeleton key={index} className="h-28 rounded-xl bg-[#172129]"/>)}</div> : data && <>
      <div className="method-strip"><ShieldCheck/><span><b>ทดสอบแบบ Conservative:</b> เข้าแท่งถัดไป · SL 1.2 ATR · TP 2R · หักต้นทุน 0.15% · ชน SL/TP แท่งเดียวให้นับ SL · สูงสุด 2 รายการ/วัน</span><em>ข้อมูลครบ {data.coverage.succeeded}/{data.coverage.requested} หุ้น</em></div>
